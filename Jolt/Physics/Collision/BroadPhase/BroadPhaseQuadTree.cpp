...
void BroadPhaseQuadTree::CollideAntiSphere(Vec3Arg inCenter, float inRadius, CollideShapeBodyCollector& ioCollector, const BroadPhaseLayerFilter& inBroadPhaseLayerFilter, const ObjectLayerFilter& inObjectLayerFilter) const
{
	JPH_PROFILE_FUNCTION();
	JPH_ASSERT(mMaxBodies == mBodyManager->GetMaxBodies());
	// Prevent this from running in parallel with node deletion in FrameSync(), see notes there
	...
	for (BroadPhaseLayer::Type l = 0; l < mNumLayers; ++l)
	{
		...
		{
			JPH_PROFILE(tree.GetName());
			tree.CollideAntiSphere(inCenter, inRadius, ioCollector, inObjectLayerFilter, mTracking);
			if (ioCollector.ShouldEarlyOut())
				break;
		}
	}
