...
void MotionProperties::SetMassProperties(EAllowedDOFs inAllowedDOFs, const MassProperties &inMassProperties)
{
	...
	// NOTE: In hypehype we just set the unmodified inertia tensor values here,
	//       this is due to undesired side effects of the modified tensors when combined with wheel constraints.
	if constexpr (true) {
		// Set inverse inertia
		Mat44 rotation;
		Vec3 diagonal;
		...
		// This is the unmodified version of the jolt code, it is kept here for reference
